...
class SubSystemIdentifierAdapter(
    private val mList: List<IdentifiersItem>,
    private val listener: FuntionalLocationFragment.itemSelected
) :
    RecyclerView.Adapter<SubSystemIdentifierAdapter.ViewHolder>() {
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        with(holder) {
            with(mList.get(position)) {
                binding.idDescription.text = this.identifier
                binding.baseCardview.setOnClickListener {
                    listener.selectedItem(mList[position].id!!, mList[position].identifierCode!!, mList[position].identifier!!)
                }
            }
        }
    }
