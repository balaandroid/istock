...
class BuisnessAdapter(
    private val mList: List<BusinessesItem>,
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        with(holder) {
            with(mList.get(position)) {
                binding.idDescription.text = this.businessName
                binding.baseCardview.setOnClickListener {
                    listener.selectedItem(mList[position].id!!, mList[position].businessCode!!, mList[position].businessName!!)
                }
            }
        }
    }
