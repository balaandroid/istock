...
class CityAdapter(
    private val mList: List<CitiesItem>,
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        with(holder) {
            with(mList.get(position)) {
                binding.idDescription.text = this.city
                binding.baseCardview.setOnClickListener {
                    listener.selectedItem(mList[position].id!!, mList[position].cityCode!!, mList[position].city!!)
                }
            }
        }
    }
