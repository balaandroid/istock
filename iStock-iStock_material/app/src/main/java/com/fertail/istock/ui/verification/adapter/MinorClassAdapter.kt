...
class MinorClassAdapter(
    private val mList: List<MinorClassesItem>,
    private val listener: FuntionalLocationFragment.itemSelected
) :
    RecyclerView.Adapter<MinorClassAdapter.ViewHolder>() {
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        with(holder) {
            with(mList.get(position)) {
                binding.idDescription.text = this.minorClass
                binding.baseCardview.setOnClickListener {
                    listener.selectedItem(mList[position].id!!, mList[position].minorCode!!, mList[position].minorClass!!)
                }
            }
        }
    }
