...
class PhysicalObsAdapter(
    private val mList: List<PhysicalObsItem>,
    private val listener: FuntionalLocationFragment.itemSelected
) :
    RecyclerView.Adapter<PhysicalObsAdapter.ViewHolder>() {
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        with(holder) {
            with(mList.get(position)) {
                binding.idDescription.text = this.physicalObservation
                binding.baseCardview.setOnClickListener {
                    listener.selectedItem(mList[position].id!!, "", mList[position].physicalObservation!!)
                }
            }
        }
    }
